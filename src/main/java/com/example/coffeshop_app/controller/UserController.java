...
import com.example.coffeshop_app.service.UserService;
...
public class UserController {
    ...
    public String adminPanel(HttpSession session, Model model) {
        // Method for displaying admin panel
        User user = (User) session.getAttribute("user");
        if (user != null && user.isAdmin()) { // Assuming isAdmin() checks if user is an admin
            model.addAttribute("user", user);
            return "admin-panel"; // Assuming "admin-panel" is your Thymeleaf template for admin panel
        } else {
            return "redirect:/login"; // Redirect to login if not admin or not logged in
        }
    }
